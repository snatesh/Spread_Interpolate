...
class SpreadTest : public ::testing::Test 
{
  protected:
    void SetUp() override
    {
      ...
      Fe_test_wrap = (double*) aligned_malloc(Nwrap * Nwrap * Nwrap * 3 * sizeof(double));
      ...
      read_from_file(Fe_wrap, Fef);
      read_from_file(flinterp, flinterpf);      
      
      init(Np, N, w, h, xp, firstn, nextn, number);
    }
    ...
    unsigned int Nwrap = w * ((int) 64 / w);
    unsigned int N = Nwrap + w;
    const double h = 1;
    double* xp;
    double* Fe_wrap;
    double* fl;
...
TEST_F(SpreadTest, SpreadAndInterpPBC)
{

  // test spread by comparing Fe_test to Fe
  spread_interp_pbc(xp, fl, Fe_test, Fe_test_wrap, firstn, nextn, number, w, h, N, true);
  for (unsigned int i = 0; i < Nwrap * Nwrap * Nwrap * 3; ++i)
  {
    ASSERT_LT(fabs(Fe_test_wrap[i]-Fe_wrap[i]),1e-14)
      << "spread value differs at index " << i 
      << " by " << fabs(Fe_test_wrap[i]-Fe_wrap[i]); 
  } 

  // test interp by comparing flinterp_test to flinterp 
  for (unsigned int i = 0; i < N * N * N * 3; ++i) {Fe_test[i] = 0;}
  spread_interp_pbc(xp, flinterp_test, Fe_test, Fe_test_wrap, firstn, nextn, number, w, h, N, false);
  for (unsigned int i = 0; i < 3 * Np; ++i)
  {
    ASSERT_LT(fabs(flinterp_test[i]-flinterp[i]),1e-14)
