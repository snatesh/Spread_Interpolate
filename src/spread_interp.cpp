                   ...
                   const double h, const unsigned short N, const bool mode)
{
  ...
  const unsigned short w2 = w * w, w3 = w2 * w; 
  const unsigned int N2 = N * N, Ndw = (unsigned int) ceil(N/w), N2dw2 = Ndw * Ndw;
  ...
  for (unsigned int izero = 0; izero < w; ++izero)
  {
    for (unsigned int jzero = 0; jzero < w; ++jzero)
    {
      ...
      for (unsigned int ijcount = 0; ijcount < N2dw2; ++ijcount)
      {
        // column indices
        unsigned int jj = jzero + w * (ijcount / Ndw);
        unsigned int ii = izero + w * (ijcount % Ndw);
        if (ii < N && jj < N)
        {
